...
if turn == 1 
    ...
    g_point = points_ss(1,:); %goal point
   ...
   if size(points_ss,1) ~= 0 
    ...
    if size(points_ss,1) ~= 0
        ...
        path_graph = addnode(path_graph,num_points_ss);  %add node to path_graph since goal already in bur so -1
    ...
    path_graph = addnode(path_graph,size(g_point,1));
    path_graph.Nodes.XData(j+1) = g_point(1);
    path_graph.Nodes.YData(j+1) = g_point(2);
    path_graph = addedge(path_graph,j,j+1);
    

    
    %STEP 3 --->now add the path from last point in points_ss to goal
    cur_par_id = find_node_id(GBur_goal,g_point);
